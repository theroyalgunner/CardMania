...
export function generateAISearchQueries(card: Partial<CollectionCard>) {
  ...
  const base = [player, year, maker, set, parallel, serial, cardNo].filter(Boolean).join(" ");

  return unique([
    base,
    [player, set, parallel, serial].filter(Boolean).join(" "),
    [player, maker, set, serial].filter(Boolean).join(" "),
    ...
    [player, maker, parallel, serial].filter(Boolean).join(" "),
